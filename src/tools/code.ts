...
export function registerCodeTools(ctx: ServerContext) {
  ...
  async function handleIndexProject(args: IndexProjectArgs) {
    ...
    try {
      ...
      currentIndex[project_name] = { path: project_path, symbols, lastIndexed: new Date().toISOString() };
  ...
  async function handleSearchSymbols(args: SearchSymbolsArgs) {
    ...
    try {
      ...
      const formatted = results.slice(0, 50).map(r => `[${r.project}] ${r.name} -> ${path.basename(r.file)}:${r.line}`).join("\n");
  ...
  async function handleReindexProject(args: ReindexProjectArgs) {
    ...
    try {
      ...
      for (const e of store) if (e.tag === CODE_TAG && e.project === project_name) existingMap.set(`${e.file}:${e.start_line}`, e);
        ...
        for (const f of files) {
          ...
          if (f.isDirectory()) { if (!CODE_SKIP_DIRS.has(f.name)) await walk(res); }
            ...
            for (let s = 0; s < lines.length; s += CODE_CHUNK_STRIDE) {
              ...
              const existing = existingMap.get(`${res}:${s + 1}`);
              if (existing && existing.hash === hash) { newEntries.push(existing); unchanged++; }
              else {
  ...
  async function handleSemanticCodeSearch(args: SemanticCodeSearchArgs) {
    ...
    try {
      ...
      const results = scored.map((e, i) => `### #${i + 1} (${e.score.toFixed(4)}) [${e.project}] ${path.relative(process.cwd(), e.file!)}\n\`\`\`\n${e.text}\n\`\`\``).join("\n\n");
